                  ...
                  >
                    ...
                    <div className="flex-1">
                      ...
                      <div className="mt-1 flex flex-wrap items-center gap-1.5">
                        <span
                          className={`inline-block rounded-full px-2 py-0.5 text-[10px] ${PRIORITY_TONE[t.priority]}`}
                        >
                          {t.priority}
                        </span>
                        {project && (
                          <span className="inline-flex items-center gap-1 rounded-full bg-card-2 px-2 py-0.5 text-[10px] text-muted-foreground">
                            <span
                        ...
                        {t.reminder && (
                          <span className="rounded-full bg-card-2 px-2 py-0.5 text-[10px] text-muted-foreground">
                            ⏰ {t.reminder}
                          </span>
                        )}
                      </div>
                  ...
                  <li key={h.id} className="premium-card rounded-2xl p-4">
                    <div className="mb-3 flex items-center gap-3">
                      ...
                      <div className="flex-1">
                        ...
                        {h.reminder && (
                          <p className="text-[10px] text-muted-foreground">⏰ {h.reminder}</p>
                        )}
                      </div>
...
function CreateModal({ kind, onClose }: { kind: MainTab; onClose: () => void }) {
  ...
  const submit = () => {
    ...
    if (kind === "tareas") {
      ...
      update((d) => ({
        ...
        goals: goalId
          ? d.goals.map((g) => (g.id === goalId ? { ...g, taskIds: [...g.taskIds, id] } : g))
          : d.goals,
      }));
    } else {
      const id = uid();
      update((d) => ({
        ...
        goals: goalId
          ? d.goals.map((g) => (g.id === goalId ? { ...g, habitIds: [...g.habitIds, id] } : g))
          : d.goals,
      }));
    }
    onClose();
  ...
  return (
    ...
    >
      <div
        className="w-full max-w-md space-y-4 rounded-t-3xl bg-card p-5 sm:rounded-3xl max-h-[92vh] overflow-y-auto"
